...
const steps = ["Scan QR", "Selfie Capture", "Liveness Check", "Sign Transaction", "Confirmed"];
        ...
        <div className="flex items-center justify-center gap-2">
          {steps.map((step, i) => (
            <div key={step} className="flex items-center gap-2">
              <div className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold transition-colors ${i <= currentStep ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                }`}>
          ...
          >
            {currentStep === 0 && (
              <div className="glass-card flex flex-col items-center p-8 text-center">
                ...
                <button
                  onClick={() => {
                    setSessionId("SESSION-TESTNET-" + Math.floor(Math.random() * 1000));
                    nextStep();
                  }}
                  className="btn-glow mt-6 rounded-lg px-6 py-2.5 text-sm font-semibold text-primary-foreground"
                >
                  Simulate Scan
                </button>
              </div>
            ...
            {currentStep === 1 && (
              <div className="glass-card flex flex-col items-center p-8 text-center">
                ...
                <button
                  onClick={() => {
                    setSelfie("data:image/jpeg;base64,mock_selfie_data");
                    nextStep();
                  }}
                  className="btn-glow mt-6 rounded-lg px-6 py-2.5 text-sm font-semibold text-primary-foreground"
                >
            ...
            {currentStep === 3 && (
              <div className="glass-card flex flex-col items-center p-8 text-center max-w-lg w-full">
                <MarkAttendance
                  sessionId={sessionId}
                  qrCode={sessionId}
                  selfieBase64={selfie}
            ...
            {currentStep === 4 && (
              <div className="glass-card flex flex-col items-center p-8 text-center">
                ...
                <p className="mt-3 font-mono text-xs text-muted-foreground">TX: stub-tx-3f8ab2c1</p>
                <button
